...
(function() {
  ...
  module.exports = RESTController = (function(superClass) {
    ...
    RESTController.prototype.index = function(req, res) {
      var done, event_data, key, opts;
      console.log('**>>> index start', this.route, req.query);
      if (req.method === 'HEAD') {
      ...
      event_data = {
        ...
        return function(err, json, status) {
          if (err) {
      ...
      if (this.cache) {
        key = "bbrindex|" + this.route + "|" + (JSON.stringify(req.query));
        opts = {};
        if (this.ttl) {
          opts.ttl = this.ttl;
        }
        return this.cache.wrap(key, ((function(_this) {
    ...
    RESTController.prototype.show = function(req, res) {
      var done, event_data, key, opts;
      console.log('**>>> show start', this.route, req.query);
      event_data = {
        req: req,
        res: res
      };
      ...
      if (this.cache) {
        key = "bbrshow|" + this.route + "|" + (JSON.stringify(req.query));
        opts = {};
        if (this.ttl) {
          opts.ttl = this.ttl;
        }
    ...
    RESTController.prototype.fetchShowJSON = function(req, res) {
      var cursor;
